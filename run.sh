...
# Check for --force / -f in arguments
FORCE=false
for arg in "$@"; do
...
for pid_file in "$LOG_DIR"/run_*.pid; do
  [ -f "$pid_file" ] || continue
  existing_pid=$(cat "$pid_file")
  if ps -p "$existing_pid" > /dev/null 2>&1; then
    if [ "$FORCE" = true ]; then
      echo "⚠ Stopping running pipeline (PID: $existing_pid)..."
      kill "$existing_pid"
      sleep 1
    else
      echo "❌ A pipeline is already running (PID: $existing_pid)"
      echo "   Log: $(ls -t $LOG_DIR/run_*.log | head -1)"
      echo "   Stop: kill $existing_pid"
      echo "   Or relaunch with --force to stop it automatically"
      exit 1
    fi
  fi
done
...
nohup "$VENV/bin/python3" -u pipeline.py \
